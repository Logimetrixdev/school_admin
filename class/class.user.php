...
class User{
	...
	function AdminLogin($runat){
        ...
        <div id="mws-login">
            <h1><img style="  border: 0 none;
    height: 81px;
    
    vertical-align: middle;
width: 243px;" src="images/logo_logi.png"/></h1>
			...
			case 'server' :
						...
						if($return){
							...
							$sql = "select * from ".TBL_ADMIN." where user='".$adminuser."' and user_type='admin' or user_type='superadmin'";
							...
							if($oldpassword == $row['password'])
								{
									...
									$this->db->update(TBL_ADMIN,$update_sql_array,'id',$_SESSION['user_id']);
	...
	function blockUser($user_id)
	{
		...
		$this->db->update(TBL_ADMIN,$update_array,'id',$user_id);
	...
	function getdatagoods($p)
					{
						$q = strtolower($_GET["q"]);
						if (!$q) return;
						$sql = "select * from ".TBL_GOODS." where partno LIKE '%$q%'";
